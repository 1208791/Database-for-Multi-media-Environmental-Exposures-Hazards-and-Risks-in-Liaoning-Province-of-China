...
def load_data_for_module(module_name):
    ...
    try:
        sheet_name = 'exposure' if module_name == 'Exposure' else 'Hazard'
        with st.spinner(f"Loading {module_name} data..."):
            ...
            if module_name == 'Exposure':
                filter_cols = ['City/Region', 'Sample type']
            elif module_name == 'Hazard':
                filter_cols = ['Media']
...
def get_filter_options(df, module_name):
    ...
    if module_name == 'Exposure':
        for col in ['City/Region', 'Sample type']:
...
def render_home_page():
    ...
    with col_m1:
        if st.button("Exposure", key="btn_exposure", use_container_width=True):
            ...
            st.rerun()
    with col_m2:
        if st.button("Hazard", key="btn_hazard", use_container_width=True):
            st.session_state['selected_module'] = 'Hazard'
            st.session_state['page'] = 'home'
            st.rerun()
    ...
    if active_module == 'Exposure':
        ...
        if df_exposure is not None:
            ...
            with col_f2:
                if 'Sample type' in options:
                    filter_values['Sample type'] = st.selectbox(
                        "Sample type",
                        options['Sample type'],
                        key="filter_sample_type",
                        label_visibility="collapsed" if 'Sample type' not in options else "visible"
                    )

    elif active_module == 'Hazard':
        # st.markdown('<h3 style="color:#1f77b4;margin:25px 0 15px 0;">Hazard Filters</h3>', unsafe_allow_html=True)
        df_hazard = load_data_for_module('Hazard')
        if df_hazard is not None:
            options = get_filter_options(df_hazard, 'Hazard')
            if 'Media' in options:
                filter_values['Media'] = st.selectbox(
                    "Media",
                    options['Media'],
                    key="filter_media"
                )
        ...
        if st.button("Help", key="help_btn", use_container_width=True):
            if active_module == 'Exposure':
                ...
                **Exposure Module Usage:**<br>
                ...
                3. (Optional) Select Sample type from dropdown<br>
                4. Click "Search" to view matching exposure data
                """
            else:
                help_text = """
                **Hazard Module Usage:**<br>
                1. Enter a CAS Number<br>
                2. (Optional) Select Media from dropdown<br>
                3. Click "Search" to view matching hazard data
                """
            st.markdown(f"""
